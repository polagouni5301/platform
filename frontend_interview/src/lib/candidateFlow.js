...
const STAGE_COOKIE_MAX_AGE = 60 * 60 * 24 * 30;
...
export const getStoredCandidateStage = () => {
  ...
  const cookieValue = document.cookie
    .split("; ")
    .find((entry) => entry.startsWith(`${CANDIDATE_STAGE_KEY}=`))
    ?.split("=")[1];
  const saved = cookieValue ?? window.localStorage.getItem(CANDIDATE_STAGE_KEY);
...
export const setStoredCandidateStage = (stage) => {
  ...
  document.cookie = `${CANDIDATE_STAGE_KEY}=${stage}; path=/; max-age=${STAGE_COOKIE_MAX_AGE}; SameSite=Lax`;
};
